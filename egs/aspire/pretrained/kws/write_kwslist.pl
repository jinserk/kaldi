...
Usage: utils/write_kwslist.pl [options] <raw_result_in|-> <kwslist_out|->
 e.g.: utils/write_kwslist.pl --flen=0.01 --duration=1000 --segments=data/eval/segments
                              --normalize=true --utter-map=data/kws/utter_map raw_results kwslist.xml
...
Allowed options:
  ...
  --keywords                  : Keywords text file                          (string,  default = "")
  --utter-id                  : Utterance id definition                     (string,  default = "")
  --utter-map                 : Map utterance for evaluation                (string,  default = "")
...
if ($keywords) {
  open(KWT, "<$keywords") || die "$0: Fail to open keywords file $keywords\n";
}

if ($utter_id) {
  open(UID, "<$utter_id") || die "$0: Fail to open utterance table $utter_id\n";
}

if ($utter_map) {
  open(UTT, "<$utter_map") || die "$0: Fail to open utterance table $utter_map\n";
}
...
if ($utter_id) {
  while (<UID>) {
    chomp;
    my @col = split(" ", $_);
    @col == 2 || die "$0: Bad number of columns in $utter_id \"$_\"\n";
    $utter_ids{$col[1]} = $col[0];
  }
}
...
if ($utter_map) {
  while (<UTT>) {
    chomp;
    my @col = split(" ", $_);
    @col == 2 || die "$0: Bad number of columns in $utter_map \"$_\"\n";
...
sub PrintKwslist {
  ...
  foreach my $kwentry (@{$KWS}) {
    ...
    if ($prev_kw ne $kwentry->[0]) {
      if ($prev_kw ne "") {$kwslist .= "  </kwentry>\n";}
      $kwslist .= "  <kwentry id=\"$kwentry->[0]\" text=\"$kwtext{$kwentry->[0]}\" search_time=\"1\" oov_count=\"0\">\n";
      $prev_kw = $kwentry->[0];
      $nbest = $nbest_max;
    }
    $nbest -= 1 if $nbest_max gt 0;
    my $score = sprintf("%g", $kwentry->[5]);
    $kwslist .= "    <kwidx file=\"$kwentry->[1]\" channel=\"$kwentry->[2]\" tbeg=\"$kwentry->[3]\" dur=\"$kwentry->[4]\" score=\"$score\" decision=\"$kwentry->[6]\"";
...
if ($keywords) {close(KWT);}
if ($utter_id) {close(UID);}
if ($utter_map) {close(UTT);}
